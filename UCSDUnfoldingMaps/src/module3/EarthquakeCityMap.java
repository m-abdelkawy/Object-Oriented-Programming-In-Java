...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {
		...
		for (int i = 0; i < earthquakes.size(); i++) {
			SimplePointMarker pointMarker = createMarker(earthquakes.get(i));
		...
		/*-------------------*/
		/*Location locCairo = new Location(30.044f, 31.23f);
		Feature pFeature = new PointFeature();
		pFeature.addProperty("title", "Egypt, Cairo");

		pFeature.addProperty("magnitude", "5.9");
		pFeature.addProperty("date", "May 22, 1992");
		pFeature.addProperty("year", "1992");

		SimplePointMarker marker = new SimplePointMarker(locCairo, pFeature.getProperties());
	...
	private SimplePointMarker createMarker(PointFeature feature) {
		...
		if (mag >= THRESHOLD_MODERATE) {
			marker.setRadius(2 * (THRESHOLD_MODERATE + 1));
			marker.setColor(red);
		} else if (mag >= THRESHOLD_LIGHT) {
			marker.setRadius(2 * THRESHOLD_LIGHT);
			marker.setColor(yellow);
		} else {
			marker.setRadius(2 * (THRESHOLD_LIGHT - 1));
