...
public class EarthquakeCityMap extends PApplet {
	...
	public void mouseMoved() {
		...
		lstMarker.addAll(quakeMarkers);
		lstMarker.addAll(cityMarkers);
		selectMarkerIfHover(lstMarker);
	}
	...
	private void selectMarkerIfHover(List<Marker> markers) {
		...
		for (int i = 0; i < markers.size(); i++) {
			if (markers.get(i).isInside(this.map, mouseX, mouseY)) {
	...
	public void mouseClicked() {
		...
		if (lastClicked != null) {
			...
			if (lastClicked != null) {
				...
				hideMarkers();
				showThreatCircle(lastClicked);
				lastClicked.setHidden(false);
				
				if(lastClicked instanceof OceanQuakeMarker) {
					((OceanQuakeMarker)lastClicked).setMap(map);
					((OceanQuakeMarker)lastClicked).setCityMarkers(cityMarkers);
				}
			}
		}
	}

	public void showThreatCircle(CommonMarker marker) {
		if (marker instanceof CityMarker) {
			for (int i = 0; i < quakeMarkers.size(); i++) {
				if (isInThreatCircle(marker, quakeMarkers.get(i))) {
					quakeMarkers.get(i).setHidden(false);
				}
			}
		} else if (marker instanceof EarthquakeMarker) {
			for (int i = 0; i < cityMarkers.size(); i++) {
				if (isInThreatCircle(cityMarkers.get(i), marker)) {
	...
	public boolean isInThreatCircle(Marker cityMarker, Marker quakeMarker) {
		//float magnitude = (Float) quakeMarker.getProperty("magnitude");
		if (cityMarker.getDistanceTo(quakeMarker.getLocation()) <= ((EarthquakeMarker)quakeMarker).threatCircle()) {
			return true;
		}
		return false;
	}
	...
	private void selectMarkerIfClick(List<Marker> markers) {
		...
		for (int i = 0; i < markers.size(); i++) {
			if (markers.get(i).isInside(this.map, mouseX, mouseY)) {
				lastClicked = (CommonMarker) markers.get(i);
				break;
			}
		}
	}
